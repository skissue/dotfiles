...
Make Howm's menu and list view properly start in Meow's motion state. Note that normally this would happen via [[help:meow-mode-state-list][meow-mode-state-list]], or, failing that, a major mode hook, but Howm does weird things when creating its buffers, and doesn't use a proper major mode, so we have to resort to advice instead 🤷.
#+begin_src elisp
  (after! meow
