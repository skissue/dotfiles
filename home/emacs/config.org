...
Make outline/Org headings sized up. Need to override Doric's default Org heading settings.
#+begin_src elisp
  (custom-set-faces
...
Change the formatting of reminders in the menu to allocate four characters for the "elapsed"/"late" number.
#+begin_src elisp
...
Include a timestamp when marking tasks as done.
#+begin_src elisp
  (el-patch-feature howm)
  (after! howm
