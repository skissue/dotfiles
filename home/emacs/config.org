...
Since this configuration is shared between desktop and Android Emacs, these two macros allow for specializing configuration to a single platform.
#+begin_src elisp
...
Convenience wrapper around [[help:require][require]] that will use Emacs' built-in package manager to install the package on Android (based on [[help:use-package-ensure-elpa][use-package-ensure-elpa]]).
#+begin_src elisp
