...
Fully trust directory-local variables within brain2:
#+begin_src elisp
...
Based on the snippets [[https://github.com/fabrizioschiavi/pragmatapro/blob/master/emacs_snippets/][provided by the PragmataPro GitHub]] to enable flexible ligature support via the built-in [[help:prettify-symbols-mode][prettify-symbols-mode]] 🎉.

Scaffolding functions to configure settings with PragmataPro's mappings.
#+begin_src elisp
...
Set up PragmataPro and enable [[help:prettify-symbols-mode][prettify-symbols-mode]] for [[help:prog-mode][prog-mode]] and [[help:text-mode][text-mode]]. The setup hook has to run first, so set its depth to one earlier than default.
#+begin_src elisp
...
"Un-prettify" symbols when point is over it or at its right edge. This has to be set /before/ enabling the mode.
#+begin_src elisp
  (setq prettify-symbols-unprettify-at-point 'right-edge)
#+end_src

** Theme
Use the [[https://github.com/dpassen/modus-alabaster][Modus Alabaster]] derivative for the Alabaster theme on top of Modus.
#+begin_src elisp
...
Alabaster discourages the use of bold/italic faces; I prefer to still have them, so override the theme.
#+begin_src elisp
...
Display buffer action to show Transients in a new popup frame. Window rules (via the compositor) are then used to position it floating at the bottom-left of the screen. Frankly, I have no idea how the =width= and =height= parameters play with the Wayland compositor, but through trial-and-error, these values seem to allow the frame to size itself to a reasonable degree.

Also, since showing a frame can be slow sometimes, delay showing a Transient menu for half a second, for the common case in which the target key is already known.
#+begin_src elisp
  (after! transient
...
*** Configuration
Configuration, with ~vertico-quick~, ~vertico-suspend~, ~vertico-repeat~, and ~vertico-directory~ extensions, as well as my own [[https://github.com/skissue/vertico-popup-frame/][vertico-popup-frame]] 🙃.
#+begin_src elisp
  (after! vertico
...
[[https://github.com/konrad1977/flyover][Flyover]] provides beautiful overlays for Flymake errors 😍.
#+begin_src elisp
...
** Uniline
Draw plain-text diagrams with Unicode! The package doesn't come with autoloads, so we have to define our own 🔧. I usually like to include full docstrings in my autoloads, but this one is just absurdly long 😅.

Bind the mode to =C-<insert>=, as suggested by the documentation; the default bind there is [[help:kill-ring-save][kill-ring-save]], which already has =M-w= 👍.
...
Show link previews using Eldoc. Part of my personal fork.
#+begin_src elisp
  (after! howm
...
Make Howm's list view ("riffle") start in Meow's motion state. This is the only mode that derives from [[help:text-mode][text-mode]] instead of [[help:special-mode][special-mode]], so it requires manual setting. Requires my personal fork.
#+begin_src elisp
  (after! meow
    (add-to-list 'meow-mode-state-list '(riffle-mode . motion)))
#+end_src

Add a new todo type for "daily" tasks. These tasks should only appear on their scheduled day, and otherwise not appear on the menu at all (minimum priority). Similar to a "schedule" item, but it sinks instantly in both directions.
#+begin_src elisp
  (after! howm
...
Patch the [[help:yank-media][yank-media]] handler (for "pasting" images and other files into attachments with a link) to not insert the file name as a description, since it is both already visible within the link itself and prevents [[help:org-link-preview][org-link-preview]] from displaying images inline.
#+begin_src elisp
...
** Org-srs
Org-srs is a complete spaced repetition system, 100% in Org Mode!
...
Add a bind to run a recipe with [[*Ghostel][Ghostel]] in justl.el.
#+begin_src elisp
  (after! justl
...
Don't default to viewing emails as =text/html= if there are other alternatives.
#+begin_src elisp
  (after! mm-decode
...
Keep the default face fixed-width in Ghostel so ligatures and other wide characters don't shift alignment.
#+begin_src elisp
  (after! ghostel
...
Automatically enable/disable [[help:ghostel-copy-mode][ghostel-copy-mode]] when entering/leaving Meow's insert mode.
#+begin_src elisp
  (after! ghostel
...
*** elfeed-protocol
Fetch feeds from my Miniflux server via [[https://github.com/fasheng/elfeed-protocol][elfeed-protocol]] and [[https://github.com/skissue/gxy/blob/main/elisp/elfeed-protocol-miniflux.el][elfeed-protocol-miniflux]].
#+begin_src elisp
  (after! elfeed
...
Command to kill the URL for the current video. Based on [[help:yeetube-play][yeetube-play]] 🙃.
#+begin_src elisp
  (after! yeetube
