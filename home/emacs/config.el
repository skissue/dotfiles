...
Calling `meow-insert-mode' when already in insert mode bugs out."
  (unless (meow-insert-mode-p)
    (meow-insert-mode)))
(defun my/meow-change-line ()
  "Kill to end of line, then enter `meow-insert-mode'."
  (interactive)
  (kill-region (point) (pos-eol))
  (my/meow-insert-enter))
(defun my/meow-yank-dwim ()
  "Do-what-I-mean when yanking: if region is active, replace it;
otherwise, yank normally."
  (interactive)
  (if (use-region-p)
      (meow-replace)
    (meow-yank)))
(defun my/meow-yank-pop-dwim ()
  "Just like `my/meow-yank-dwim', but for `meow-yank-pop' instead."
  (interactive)
  (if (use-region-p)
      (meow-replace-pop)
    (meow-yank-pop)))
(dotimes (i 10)
  (let ((sym (intern (format "my/meow-%i-dwim" i)))
        (expand-sym (intern (format "meow-expand-%i" i))))
    (eval `(defun ,sym ()
             "Expand selection if active, otherwise invoke
 universal argument with digit."
             (interactive)
             (if (use-region-p)
                 (,expand-sym)
               (meow-digit-argument))))))
(defun my/meow-reverse-dwim ()
  "Call `meow-reverse' if a region is active, otherwise `negative-argument'."
  (interactive)
  (if (use-region-p)
      (meow-reverse)
...
(setcdr (assq 'meow-kill meow-selection-command-fallback)
        #'meow-delete)
